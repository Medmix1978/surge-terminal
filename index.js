...
class DataClient {
  ...
  async getPortfolio() {
    if (!this.kraken.apiKey) {
      console.log('[PORTFOLIO] No API key — returning empty portfolio');
      return { totalEquity: 0, unrealizedPnl: 0, balances: [], positions: [], live: false };
    }
    try {
      console.log('[PORTFOLIO] Fetching real Kraken balance...');
      const balances = await this.kraken.getBalances();
      const tradeBalance = await this.kraken.getTradeBalance();
      console.log('[PORTFOLIO] Balance OK — equity:', tradeBalance?.eb, 'pnl:', tradeBalance?.n);
      return { totalEquity: parseFloat(tradeBalance?.eb || '0'), unrealizedPnl: parseFloat(tradeBalance?.n || '0'), balances, positions: (db.trades || []).filter(t => t.status === 'open').map(t => ({ pair: t.pair, side: t.side, entryPrice: t.price, currentPrice: t.price, pnl: t.pnl || 0 })), live: true };
    } catch (err) {
      console.error('[PORTFOLIO] Kraken private API failed:', err.message);
      return { totalEquity: 0, unrealizedPnl: 0, balances: [], positions: [], live: false };
    }
  }
}
...
function loadConfig() {
  ...
  return {
    ...
    minTradeAmount: parseFloat(cfg.minTradeAmount) || 100,
    ...
    demoMode: cfg.demoMode === 'true' || cfg.demoMode === true,
...
function saveConfig(config) {
  db.bot_config = {
    ...
    minTradeAmount: String(config.minTradeAmount || 100),
    ...
    demoMode: String(config.demoMode || false),
  };
  saveDb(db);
}
...
// Load saved keys from DB on startup (e.g. after Render restart)
const savedKeys = db.api_keys || {};
if (savedKeys.kraken_api_key && savedKeys.kraken_api_secret) {
  console.log('[STARTUP] Loading saved Kraken keys from database...');
  kraken.apiKey = savedKeys.kraken_api_key;
  kraken.apiSecret = savedKeys.kraken_api_secret;
  dataClient.kraken.apiKey = savedKeys.kraken_api_key;
  dataClient.kraken.apiSecret = savedKeys.kraken_api_secret;
  console.log('[STARTUP] Kraken keys loaded — private API ready');
} else {
  console.log('[STARTUP] No saved Kraken keys found — public API only');
}
...
app.get('/api/balance', async (req, res) => {
  try {
    if (!kraken.apiKey) {
      return res.status(400).json({ success: false, error: 'No Kraken API keys configured. Connect keys first.' });
    }
...
// Scheduled signal generation + auto-trade execution
cron.schedule('*/5 * * * *', async () => {
  ...
  try {
    ...
    if (newSignals.length > 0) {
      console.log(`[CRON] ${newSignals.length} new signals generated`);
      wsManager.broadcast('signals', newSignals);

      // AUTO-TRADE EXECUTION
      if (config.autoTrade) {
        console.log(`[AUTO-TRADE] Auto-trading ENABLED — evaluating ${newSignals.length} signals...`);
        const keys = db.api_keys || {};
        if (!keys.kraken_api_key) {
          console.log('[AUTO-TRADE] SKIPPED — no Kraken API keys configured');
        } else {
          for (const signal of newSignals) {
            // Only execute HIGH confidence signals
            if (signal.impact !== 'HIGH' || signal.confidence < 70) {
              console.log(`[AUTO-TRADE] SKIPPED ${signal.pair} ${signal.side} — confidence ${signal.confidence.toFixed(0)}% too low (needs 70%+ HIGH)`);
              continue;
            }

            // Determine volume based on minTradeAmount and current price
            const ticker = tickers.data.find(t =>
              (signal.pair === 'ETH/USD' && (t.pair.includes('ETH') || t.pair.includes('XETH'))) ||
              (signal.pair === 'BTC/USD' && (t.pair.includes('XBT') || t.pair.includes('BTC')))
            );
            if (!ticker || !ticker.last) {
              console.log(`[AUTO-TRADE] SKIPPED ${signal.pair} — no price data`);
              continue;
            }

            const minAmount = config.minTradeAmount || 100;
            const volume = (minAmount / ticker.last).toFixed(6);
            const krakenPair = signal.pair === 'ETH/USD' ? 'XETHZUSD' : 'XXBTZUSD';

            try {
              console.log(`[AUTO-TRADE] EXECUTING ${signal.side} ${volume} ${krakenPair} @ ~$${ticker.last.toFixed(2)}`);
              ...
              console.log(`[AUTO-TRADE] SUCCESS ${krakenPair} — txid:`, result.txid);
              ...
              console.error(`[AUTO-TRADE] FAILED ${krakenPair}:`, execErr.message);
            }
          }
        }
      } else {
        console.log('[AUTO-TRADE] Auto-trading DISABLED — signals generated but not executed');
      }
    } else {
      console.log('[CRON] No signals generated this cycle');
    }
